...
| Part of RISK-035 | The Jest environment is repaired, #124 adds the complete frontend suite as a named blocking hosted step, and #126 adds the standalone SPA callback suite. CI-001B3 [#167](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/167) merged the lockfile-bound Node 20/Java 17 `Commerce command journal emulator` job using exact demo project `demo-pay002b2-test`, Firestore only, and explicit opt-in. Static negative tests require that exact job at both protected-release CI rechecks. | #169 expands the same focused suite for lease/takeover/stale-fence/terminal races. A green demo-emulator job proves synthetic source behavior only; it does not deploy Firebase or prove Stripe/live behavior. Make lint non-mutating/fail-closed, configure and prove branch-required checks, and add the broader TEST-001 suite. |
