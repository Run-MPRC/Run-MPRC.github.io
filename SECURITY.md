...
| RISK-026, RISK-036 | #135 replaces automatic frontend-first/fail-open GitHub deployment with a tested manual exact-commit gate. It pins release Actions, uses the lockfile Firebase CLI, validates protected configuration before backend install/authentication, deploys fixed Rules/Functions before Pages publication, removes cloud credentials from website preparation/publication, and stops Git-triggered Netlify production builds. | #133 must configure protected environments and least-privilege OIDC/WIF; #136 must prove staging/target deployment; WEB-001 must establish protected Netlify publication and rollback. No Firebase, Pages, or live-host deployment is proven by #135 source/tests. |
