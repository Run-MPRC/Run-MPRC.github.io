...
| Part of RISK-027 | #99 blocks App Check, Firebase Analytics, and Sentry in local/test and on initial capability-callback startup. OBS-001A1 [#134](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/134) source disables Sentry replay, tracing, browser session tracking, client reports, member user context, breadcrumbs, attachments, transactions, and unapproved default integrations. Its fail-closed allowlist retains only bounded low-cardinality event fields, generic exception types, and compiled-bundle path/line metadata; synthetic tests use a mocked Sentry client and make no provider call. | #111 still owns Firebase Analytics allowlisting and any residual hosted telemetry boundary. Source/tests and merge evidence do not prove website publication, the exact `runmprc.com` revision, Sentry transport/ingestion or provider IP handling, retention, access, consent, deletion, or vendor terms; verify those separately under #110. Callback protected handoffs remain with ABUSE-001A, DATA-001A, and OAUTH-001C in #88. |
