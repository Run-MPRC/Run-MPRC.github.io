...
| RISK-035 | Frontend tests currently fail because `TextEncoder` is absent in Jest; CI's frontend lint step is allowed to fail with `|| true`; Functions have only seven unit tests and about 20% line coverage. | Repair the harness, make non-mutating lint/test gates mandatory, and add domain/integration coverage. |
