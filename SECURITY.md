...
| RISK-035 | The deterministic frontend Jest suite and standalone SPA callback suite run as separate blocking hosted CI steps. CI-001B4/#186 adds a non-mutating frontend lint gate over 102 JS/JSX/TS/TSX files. Its reviewable baseline records every one of the 125 configured errors and 7 warnings that remain after CI disables the repository's severity-masking `eslint-plugin-only-warn` hook. Branch protection, lint-debt cleanup, and broader domain/integration coverage remain incomplete. | Reduce the reviewed finding baseline in focused changes, prove required branch checks, and add domain/integration coverage. Never regenerate the baseline merely to make CI green. |
...
| Part of RISK-035 | The Jest environment is repaired, #124 adds the complete frontend suite as a named blocking hosted step, and #126 adds the standalone SPA callback suite. CI-001B3 [#167](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/167) merged the lockfile-bound Node 20/Java 17 `Commerce command journal emulator` job using exact demo project `demo-pay002b2-test`, Firestore only, and explicit opt-in. CI-001B4 [#186](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/186) tracks a no-new-dependency exact-record checker over 102 frontend files, a named blocking hosted lint step, and a clean-checkout step. It disables `eslint-plugin-only-warn` in the lint process so original configured severities remain visible. Its static tests reject skipped, renamed, mutating, incomplete, conditional, lifecycle-hooked, execution-context-overridden, or failure-swallowing wiring. | The reviewable lint baseline deliberately records 125 configured errors and 7 warnings as individual file/line/column/rule/message records; it is not a clean-lint claim. Any added, removed, moved, or changed record requires review, every fatal parser/configuration failure rejects with a fixed diagnostic, and the baseline must not be regenerated just to pass. The clean check covers tracked and non-ignored untracked files; ignored dependency/build outputs are outside this proof. #169, #173, and #182 expand the focused commerce suite. Green synthetic jobs do not deploy Firebase or prove Stripe/live behavior. Configure/prove branch-required checks and add the broader TEST-001 suite. |
