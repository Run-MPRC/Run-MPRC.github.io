...
**Current partial boundary:** PAY-001A is complete in [#157](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/157). PAY-001B1 is tracked in [#219](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/219): the website projects only the active field set and omits volunteer tier; after App Check and fail-closed static configuration, the current race endpoint adopts an exact bounded request envelope before Firestore or commerce-control reads, preserves opaque event identity, encodes callback values, and matches answers to the admitted event's bounded selected field definitions before any mutable or provider side effect. PAY-001C1B [#389](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/389) adds the first merchandise-checkout adoption: the endpoint strict-parses the exact request root and buyer before Firestore, rate limits, or Stripe, then matches the size and color selections against the stored catalog option lists before the price projection or Stripe, while the website sends only offered options. PAY-001B2 still owns immutable price, waiver, and field-definition snapshots plus compatibility and protected staging proof. The rest of PAY-001C (immutable merchandise price snapshots and multi-quantity), PAY-001D, deployment, and live protection remain **NOT AVAILABLE YET**.
