...
describe('stripeWebhook', () => {
  ...
  test('quarantines a paid Session with an unapproved promotion discount', async () => {
    seedRegistration();

    await deliver(stripeEvent(
      'evt_unapproved_discount',
      'checkout.session.completed',
      registrationSession({
        amount_total: 4500,
        total_details: {
    ...
    expect(admin.__get('stripeEvents/evt_unapproved_discount')).toMatchObject({
  ...
  test.each([
    ['tax', { amount_tax: 100, amount_shipping: 0 }, 2100, 'tax_not_configured'],
    [
      'shipping charge',
      { amount_tax: 0, amount_shipping: 250 },
      2250,
      'shipping_charge_not_configured',
    ],
  ])('quarantines an unconfigured %s', async (_label, adjustments, total, reason) => {
    seedOrder();

    await deliver(stripeEvent(
      `evt_unconfigured_${reason}`,
      'checkout.session.completed',
      orderSession({
        amount_total: total,
        total_details: { amount_discount: 0, ...adjustments },
      }),
    ));
  ...
  test('does not let an older partial-refund event downgrade a full refund', async () => {
    ...
    expect(admin.__get('orders/order-1')).toMatchObject({
      ...
      stripeRefundIds: ['re_full', 're_partial'],
    });
