...
The optional profile photo and officer **People finder** are also **NOT AVAILABLE YET**. #505/#506 source keeps the choice off by default and limits the source-only officer page to explicit name search with voluntary thumbnails. It is not a membership roster and does not recognize faces. Do not open production Account/Admin pages, upload a real photo, search a real name, or treat a source merge or preview as live proof. Follow the source-only procedure in [Events, shop, members, and money](./docs/officers/EVENTS_SHOP_MEMBERS.md).
