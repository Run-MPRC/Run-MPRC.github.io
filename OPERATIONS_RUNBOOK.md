...
WEB-002A [#473](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/473) is a temporary web-only permissions-containment release and is **PENDING REVIEW AND RELEASE**. Its active manifest accepts only a two-parent control merge whose first parent is `dee79511b6e371329aa129139729e112e7a51aad`, then fetches dedicated source ref `codex/netlify-source-473-permissions-containment-v2` at commit `39ab8649df411262c8109a3c81a57bc38f1e168b` and tree `d76b496cdc5e79015bc048cb961758abbe88b9ce`. That source is based directly on live artifact source `ed1b0833f25822cee80c99ded8753722b5608a3f`; its executable delta is limited to the static Hat/Jacket Shop and fixed accessible Events/Calendar failure messages. Two synthetic tests cover those runtime changes. The build must reproduce exactly 60 files and digest `07b10c7d5ff176a1ad893d7549b07042312df35f4a4126e8765824ca94eaefb8`; current production deploy `6a6dc219a8136300081811db` is the rollback target. Before merge, prepare both the manifest-disable follow-up and an exact Git rollback projection, freeze unrelated `main` merges, re-read the issue and pull request for new blockers, and compare the executable delta with the live artifact rather than trusting provenance alone. Verify the exact preview and live site separately: marker values; signed-out `/shop`, `/events`, `/events/calendar`, and `/account` at phone and computer widths; HTTP 200 for `/shop/mprc-hat` and `/events/calendar`; marker HTTPS, no-store, JSON, nosniff, and HSTS headers; preview noindex; `/robots.txt` HTTP 200 with `/admin` and `/login` disallowed; and no new browser-console error. A fixed retry-later notice on Events/Calendar is the expected containment result; it does not restore event records. This release deploys no Firebase, Rules, Functions, provider configuration, account state, protected offer, officer editor, ledger, or production data. After successful live verification, disable the manifest and retire the release source; if publication fails or any public proof differs, keep or restore the current deploy.
