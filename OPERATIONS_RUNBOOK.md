...
| Staging | `run-mprc-staging` | Empty Firestore Rules/index boundary only; no Auth, Functions, App Check, or provider sandbox | `run-mprc-staging.web.app`; `dev.runmprc.com` not configured | `staging`: two reviewers, exact `main`, read-only OIDC verifier | Static signed-out Hosting, protected project-identity proof, and empty delete-protected Firestore are available; usable backend/provider staging remains a launch blocker |
...
`firebase.json` has a Hosting block for `build/` and missing-path SPA rewrites. Its predeploy chain rejects a Firebase CLI project that differs from the selected browser project, then rebuilds and scans executable JavaScript before upload. **Static engineering staging, a protected read-only authority check, and an empty Rules-protected Firestore boundary are available; a usable backend and production publication are NOT AVAILABLE YET.** #665 used that chain for the first `run-mprc-staging` Hosting release. #667's WIF identity can read only the project identity; it cannot deploy. #669 was a separate bounded database/Rules/index operation and did not grant reusable authority. No protected Firebase deploy role, Auth test setup, Function, App Check enforcement, provider sandbox, production authority, custom domain, release marker, tested rollback, production release, or DNS/TLS change exists.
