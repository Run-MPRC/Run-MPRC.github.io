...
### Profile thumbnail and officer people finder — SOURCE ONLY, NOT LIVE

Issue [#505](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/505) adds the private profile-photo/default-off-preference slice of the optional people finder in [#504](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/504). Issue [#506](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/506) adds the minimum server-only projection, current-state reconciler, bounded officer name search, and separate gallery. Do not deploy or publish either slice independently. Neither issue authorizes real photos, real-name searches, or production testing.

Source review must prove all of these with generated non-face images and made-up accounts:
...
#506 source review must also prove all of these with made-up names and generated non-face images:
...
The approved release order remains blocked on [#507](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/507), the #110 privacy decision, protected short-lived deployment authority under #133, and isolated staging. When those dependencies are complete, deploy and verify the exact Rules plus `getMyMemberDirectoryProfile`, `setMyMemberDirectoryVisibility`, `setMyMemberDirectoryPhoto`, `removeMyMemberDirectoryPhoto`, `searchMemberDirectory`, and `syncMemberDirectoryEntryOnMemberWrite` before publishing the dependent website. Deploy and read back any required index with that backend set, and verify the trigger's retry policy. Before initial publication, prove that no older preference rows need projection or run an approved idempotent, dry-run-first current-state repair; retain that owned repair path for schema/normalization upgrades. A skipped or partial backend publishes nothing. Verify default-hidden, upload-without-opt-in, opt-in name search, opt-out absence, name change, stale-entry withholding, and photo removal with synthetic records only. Never test with a real member photo, inspect production profiles, widen Rules, or repair a record manually.
