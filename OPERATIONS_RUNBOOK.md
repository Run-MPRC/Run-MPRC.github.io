...
The bounded #676 provider checkpoint ran on 2026-08-26 only after exact source `bce911a7083201cfe2141edaa9660f210287bf57` reached `main` and all five jobs in post-main CI run `33018136439` passed. It enabled only Firebase App Check and reCAPTCHA Enterprise, created one score-based key restricted to `run-mprc-staging.web.app` and `run-mprc-staging.firebaseapp.com`, registered it to the one staging web app with a one-hour token lifetime, and atomically set Authentication plus Firestore to `UNENFORCED` with replay protection off. The guarded Hosting-only command published the same exact source as version `8556fc51210bdc66`; the 61-file artifact manifest SHA-256 is `157e2048aa86d45cec1111e091643870d18ced808ef88d610ec70aae1bbe1ace`. Root and direct `/events` returned 200; the empty Events state and 1280-by-720 layout rendered correctly; browser warnings/errors were absent. Provider readback still showed billing disabled, zero Auth users, zero Firestore root collections, and no Functions-related API or Function. This is a monitoring checkpoint, not enforcement evidence: do not claim completion until the new disposable Auth browser flow, `ENFORCED` readback, missing-token denial, token-bearing replay, final cleanup, and evidence merge all pass. If a post-enforcement browser check fails, atomically restore both services to `UNENFORCED`, verify that readback, and preserve the restricted provider configuration for diagnosis.
...
`firebase.json` has a Hosting block for `build/` and missing-path SPA rewrites. Its predeploy chain rejects a Firebase CLI project that differs from the selected browser project, then rebuilds and scans executable JavaScript before upload. It also has the #671 email/password-only Auth source block; source remains configuration intent, while the separate provider evidence proves the live narrow Auth boundary. **Static engineering staging, a protected read-only authority check, an empty Rules-protected Firestore boundary, disposable email/password Auth proof, and an App Check monitoring checkpoint are available; a usable backend and production publication are NOT AVAILABLE YET.** #665 used the Hosting chain for the first `run-mprc-staging` release. #667's WIF identity can read only the project identity; it cannot deploy. #669, #671, and the #676 monitoring checkpoint were bounded provider operations and did not grant reusable authority. Exact source `bce911a7083201cfe2141edaa9660f210287bf57` is current staging Hosting version `8556fc51210bdc66`; Authentication and Firestore remain `UNENFORCED` with replay protection off. No protected Firebase deploy role, Function, enforced App Check service, provider sandbox, production authority, custom domain, release marker, tested rollback, production release, or DNS/TLS change exists.

### App Check release state — MONITORING AVAILABLE; ENFORCEMENT NOT AVAILABLE YET
...
The later #676 monitoring checkpoint is now live only on `run-mprc-staging`. It has one score-based Enterprise key restricted to the two staging hosts, one matching web-app config with a one-hour token lifetime, exact source `bce911a7083201cfe2141edaa9660f210287bf57` published as Hosting version `8556fc51210bdc66`, and Authentication plus Firestore set to `UNENFORCED` with replay protection off. Public Hosting and empty Firestore/UI checks passed. The new disposable browser Auth flow, enforced readback, missing-token denial, and token-bearing replay have not passed yet; do not describe monitoring as protection.

The safe sequence is now at the browser-proof step:
...
5. With explicit approval for the live browser credential submission, prove one made-up disposable Auth sign-up/sign-in/sign-out plus the empty Firestore read while both services remain in monitoring mode. Delete the identity afterward.
6. Atomically set only staging Authentication and Firestore to `ENFORCED`, leave replay protection off, wait for propagation, then prove missing-token denial and continued token-bearing browser success. If the browser fails, atomically restore both services to `UNENFORCED` and verify the rollback.
7. Reconfirm zero users and root collections, billing disabled, no Functions API/deployment, and production unchanged before merging completion evidence.
