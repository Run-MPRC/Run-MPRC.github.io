...
WEB-002A [#473](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/473) is a new temporary web-only release and is **PENDING REVIEW AND RELEASE**. Its active manifest accepts only a two-parent control merge whose first parent is `52fd426662c3d814044aa55eb6104ea1464a5bc6`, then fetches dedicated source ref `codex/netlify-source-473-shop-events-auth` at commit `094af1096ed8721597561cd59bf695d4c4a9d210` and tree `d12fd7660beaaddac5e952fe950dc6ad6bbdf68f`. The build must reproduce exactly 59 files and digest `72f16455fb67b0f00408ed867b69543c94395fb28aaa11cb2d490383de1aed01`; prior production deploy `6a61c544171ea80008307623` remains the rollback target. Its public marker identifies prior artifact source `ed1b0833f25822cee80c99ded8753722b5608a3f`, and dedicated rollback ref `codex/netlify-source-473-rollback` pins that exact source. Before merge, prepare both the manifest-disable follow-up and a Git rollback projection, then freeze unrelated `main` merges. Verify the exact preview and live site separately: marker values; signed-out `/shop`, `/events`, and `/login` at phone and computer widths; HTTP 200 for `/shop/mprc-hat` and `/events/calendar`; marker HTTPS, no-store, JSON, nosniff, and HSTS headers; preview noindex; `/robots.txt` HTTP 200 with `/admin` and `/login` disallowed; and no new browser-console error. This release changes only static website files. It deploys no Firebase, Rules, Functions, provider configuration, account state, protected offer, officer editor, ledger, or production data. After successful live verification, disable the manifest and retire the release source; if publication fails or any public proof differs, keep or restore the prior deploy.
