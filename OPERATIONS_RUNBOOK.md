...
| Staging | `run-mprc-staging` | Empty Firestore Rules/index boundary plus deployed password-required email/password Auth and enforced App Check for Auth/Firestore; source-only exact two-Function guard; no Functions or provider sandbox | `run-mprc-staging.web.app`; `dev.runmprc.com` not configured | `staging`: two reviewers, exact `main`, read-only OIDC verifier | Static Hosting, protected project-identity proof, empty delete-protected Firestore, disposable Auth/App Check proof, and Function command guard are available; billing and usable backend/provider staging remain launch blockers |
...
WEB-001A2 [#665](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/665) created and provider-read back `run-mprc-staging`, one web app, and Hosting version `3ffadcf2ac8cc760` from source `a614c68d9a1a2be631a3be874a686d61e5d170a0` on 2026-08-26. Always name that project explicitly; `.firebaserc` still defaults to the demo namespace. The static host is not complete environment isolation.

CI-001C1 [#667](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/667) adds a separate staging-only authority check. The `staging` environment admits exact `main` and requires one of two named individual organization administrators. The OIDC provider additionally requires immutable organization/repository IDs, the environment, ref, and one verifier workflow. Its keyless service account has no user-managed key and only `resourcemanager.projects.get`; it has no Firebase deployment role. The verifier performs one fixed read of the exact active project identity and emits no response body. This proof does not enable or deploy Auth, Firestore, Rules, indexes, Functions, Hosting, App Check, Storage, an outside provider, or production.

CI-001D1 [#669](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/669) provisioned only the staging `(default)` Firestore database and deployed exact Rules/indexes from source `ee16bd16220ab58bd3a2add80dd2f39a1d514dd7`. Readback on 2026-08-26 showed Native mode, Standard edition, `us-west2`, delete protection enabled, PITR disabled, Rules SHA-256 `fe3f53302db76df5febf7e9d242d82a6b6bec0c0b574ecc8253f1034f1a77dba`, four matching composite indexes, 28 matching field overrides, and zero root collections. Anonymous reads of fixed nonexistent `mail`, `stripeEvents`, and `auditEvents` documents returned `403`. The Rules emulator suite passed 418/418 before provider mutation. No application record, Auth provider/test user, Function, App Check setting, Storage data, outside provider, Hosting version, production Firebase surface, or website changed.

CI-001D2 [#671](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/671) first adds the source contract for Firebase Authentication: only `emailPassword: true`, with no anonymous, Google, or phone provider in source. `npm run deploy:staging-auth` is fixed to Firebase CLI 15.24.0, `--only auth`, and `run-mprc-staging`; its lifecycle guard also requires that exact cloud/quota project, the declared `runmprc@gmail.com` account, a short-lived `FIREBASE_TOKEN`, no ADC file override, and the unchanged demo default alias. Do not run that command from a branch. Merge and verify the exact source first.

The bounded provider phase ran on 2026-08-26 from exact merged source `42542303d043f87a8f1a04be2f0b4f2a88e0318c` after all five required jobs in CI run `33011780449` passed. Live readback showed the instrumentless `IDENTITY_PLATFORM` subtype with billing disabled; password-required email/password and improved email privacy were enabled; authorized domains were exactly `run-mprc-staging.firebaseapp.com` and `run-mprc-staging.web.app`; and anonymous, phone, native federated, OIDC, and SAML providers were absent. A disposable reserved-domain user passed API create/sign-in, generic invalid-credential, anonymous-denial, and browser sign-in/sign-out checks. No out-of-band email or SMS was requested. Cleanup returned Auth to zero users and Firestore to zero root collections. At that #671 checkpoint, Hosting remained version `3ffadcf2ac8cc760`, Functions and App Check were unavailable, and production was untouched; #676 separately records the later Hosting version and current App Check enforcement. This evidence is not a reusable officer deployment control.
...
CI-001D4 [#676](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/676) adds `npm run deploy:staging-hosting` as a separate source-only, argument-closed wrapper. It checks the demo-only default alias, exact Hosting predeploy chain, lockfile CLI 15.24.0, exact staging project/quota/account, Hosting-only scope, in-memory short-lived token, no credential-file override, and a bounded non-placeholder Enterprise site key. The executable scanner must find that same public key in the built JavaScript. Do not run it from a branch, append arguments, use a debug token, add localhost to provider domains, or treat the public site key as a server credential.

The bounded #676 provider phase ran on 2026-08-26 only after exact source `bce911a7083201cfe2141edaa9660f210287bf57` reached `main` and all five jobs in post-main CI run `33018136439` passed. It enabled only Firebase App Check and reCAPTCHA Enterprise, created one score-based key restricted to `run-mprc-staging.web.app` and `run-mprc-staging.firebaseapp.com`, registered it to the one staging web app with a one-hour token lifetime, and published the same exact source as Hosting version `8556fc51210bdc66`; the 61-file artifact manifest SHA-256 is `157e2048aa86d45cec1111e091643870d18ced808ef88d610ec70aae1bbe1ace`. Root and direct `/events` returned 200; the empty Events state and 1280-by-720 layout rendered correctly; browser warnings/errors were absent. A made-up reserved-domain browser identity then completed account creation, sign-out, fresh sign-in, and post-enforcement sign-in. Authentication plus Firestore were atomically changed from `UNENFORCED` to `ENFORCED` with replay protection `OFF`; a valid-credential Auth request without App Check returned HTTP 401, and a tokenless Firestore Events request returned HTTP 403. Token-bearing Events reads continued to render the empty result signed in and signed out. The disposable identity was deleted; final provider readback showed zero Auth users, zero Firestore root collections, billing disabled, and no Functions-related API or Function. Preserve `ENFORCED`/`OFF`; if a future browser regression appears, open an incident before changing policy and use the same atomic two-service rollback only with platform/security approval.
...
`firebase.json` has a Hosting block for `build/` and missing-path SPA rewrites. Its predeploy chain rejects a Firebase CLI project that differs from the selected browser project, then rebuilds and scans executable JavaScript before upload. It also has the #671 email/password-only Auth source block; source remains configuration intent, while separate provider evidence proves the live narrow Auth boundary. **Static engineering staging, a protected read-only authority check, an empty Rules-protected Firestore boundary, disposable email/password Auth proof, and enforced App Check for Auth plus Firestore are available; Functions, a usable member backend, and production publication are NOT AVAILABLE YET.** #665 used the Hosting chain for the first `run-mprc-staging` release. #667's WIF identity can read only the project identity; it cannot deploy. #669, #671, and #676 were bounded provider operations and did not grant reusable authority. Exact source `bce911a7083201cfe2141edaa9660f210287bf57` is current staging Hosting version `8556fc51210bdc66`; Authentication and Firestore read `ENFORCED` with replay protection `OFF`. No protected Firebase deploy role, Function, provider sandbox, production authority, custom domain, release marker, tested rollback, production release, or DNS/TLS change exists.

### App Check release state — ENFORCEMENT AVAILABLE IN STAGING
...
Protected release `29254280177` stopped before checkout/build because `REACT_APP_RECAPTCHA_SITE_KEY` was absent. Nothing was published in that attempt. Do not use a placeholder, reuse an unverified v3 key, expose a debug token, or weaken the release check.

The later #676 App Check boundary is live only on `run-mprc-staging`. It has one score-based Enterprise key restricted to the two staging hosts, one matching web-app config with a one-hour token lifetime, exact source `bce911a7083201cfe2141edaa9660f210287bf57` published as Hosting version `8556fc51210bdc66`, and Authentication plus Firestore set to `ENFORCED` with replay protection `OFF`. Disposable browser Auth, token-bearing signed-in/signed-out Events reads, tokenless Auth/Firestore denial, cleanup, and final provider readback passed. Do not generalize that proof to absent Functions, private roles/data, outside providers, production, or real identities.

The #676 sequence is complete. Preserve the proved boundary:

1. Keep the exact project/account/host inventory and zero-user/zero-record checks current.
2. Preserve the merged #159 Enterprise browser source and #676 argument-closed Hosting guard.
3. Preserve the one score-based key restricted to only the two staging hosts and its one-hour registration to the one staging web app.
4. Preserve exact Hosting source `bce911a7083201cfe2141edaa9660f210287bf57` and version `8556fc51210bdc66`; do not rebuild from a branch or widen the Hosting-only scope.
5. Preserve Authentication and Firestore as `ENFORCED` with replay protection `OFF`; the approved made-up browser lifecycle, tokenless denial, and token-bearing Events evidence passed.
6. If a future browser regression appears, stop staging sign-in use and open an incident before changing policy. Roll back both services atomically only with platform/security approval, then verify the readback.
7. Reconfirm zero users and root collections, billing disabled, no Functions API/deployment, and production unchanged before any later staging release.
8. Keep native callable enforcement and Function deployment separate until the billing gate is explicitly approved; publish and verify production website, Firebase, provider, and live behavior separately under #136/WEB-001.
...
Never print secret values into CI output, issue bodies, screenshots, chat, or this runbook. With Firebase CLI 15.24.0, do not run `firebase login:list --json` in any captured or shared session because its account objects can contain OAuth tokens. Use a non-token identity check. If token-bearing output appears, stop, revoke every affected OAuth grant, and record only the redacted incident result.
...
### Authorized Stripe attempt-2 plan — SOURCE ONLY, UNUSED

PAY-002B2C4A [#232](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/232) adds one server-only attempt-2 plan after the C3C authorization. No endpoint or Functions index imports it. It cannot create pre-send evidence, call Stripe, change a registration/order, return a Checkout URL, or send a response to a member.

```mermaid
flowchart LR
    A["Exact C3C authorization + audit"] --> G{"Exact chain and active lease?"}
...
Expected proof: both allowed C3C transitions bind the same constrained attempt-2 plan shape; every provider-field drift fails before writes; exact concurrency creates one pair; exact retry and a later valid lease only observe it; prior records remain byte-identical; malformed partners and browser access fail closed.
...
The GitHub release workflow is now manual and exact-current-commit. After protected approval it rechecks `main`, the newest exact CI run, and the retained credential-free artifact before cloud authentication. It fails on missing protected configuration, uses a lockfile Firebase CLI, verifies reviewed Rules and two named profile Functions before publishing the Pages branch, and never gives cloud authority to website preparation or publication. Future Pages artifacts omit the `runmprc.com` CNAME, but the existing Pages provider setting still claims that Netlify-served name until #136/WEB-001 publish and read it back. #667's separate staging verifier proves protected short-lived authentication only; its principal has no deployment permission and `.github/workflows/deploy.yml` still rejects staging. #669 proves the exact Rules/index content can be applied to the empty staging database, but it adds no reusable deploy authority and no Function/Auth/App Check/provider boundary. The release source gate remains unavailable until #133 supplies the remaining staging deployment roles and protected production environment/identity. It does not publish the live Netlify host. Treat the full pipeline as incomplete until #105 closes.

WEB-PRIVACY-001AA [#582](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/582) changes only the executable Netlify release builder's outer caught-failure output. Source now uses an unbound catch, prints fixed `netlify_release_build_failed`, and preserves the failing exit. Synthetic black-box tests cover ordinary message detail, a hostile message getter, and a hostile prototype trap before any provider or release work can start. This proves only that the wrapper does not inspect or echo its caught value. At the #582 boundary, the child-process runner and two Git reads still inherited subprocess diagnostics; #596 below closes that separate residual inside the builder. The success evidence line, cleanup, authorization, build, artifact, and publication behavior are unchanged. Source, tests, and any merge do not prove a Netlify publication, website change, `runmprc.com` verification, Firebase deployment, provider configuration, production-data change, or live behavior; the fixed code says only that preparation failed.

WEB-PRIVACY-001AB [#596](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/596) closes that named child-output residual inside the executable builder. Generic Git, npm, tar, and frontend-build commands now ignore child stdin, stdout, and stderr. The two Git identity reads capture only the required commit or tree on stdout and discard stderr. Synthetic local executables prove that ordinary exit failure, signal termination, `rev-parse` failure, `show` failure, and invalid captured identity values leave parent stdout empty and produce only fixed `netlify_release_build_failed` on parent stderr. This intentionally removes child progress and diagnostic detail. It preserves failure detection, exact Git comparison, cleanup, artifacts, and the parent-owned final `Prepared …` evidence. It does not sanitize Netlify platform output outside this process. Source, tests, and any merge still prove no website publication, `runmprc.com` verification, Firebase deployment, Netlify/provider configuration, production-data change, or live behavior.
...
WEB-002A [#473](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/473) completed its bounded web-only permissions containment on 2026-08-01. Exact two-parent merge `9ad6837756cdd409d296009fde5082eeeae5c059` published Netlify deploy `6a6dc9ea588b0c0008036312` from frozen source `39ab8649df411262c8109a3c81a57bc38f1e168b`, tree `d76b496cdc5e79015bc048cb961758abbe88b9ce`, 60 files, and digest `07b10c7d5ff176a1ad893d7549b07042312df35f4a4126e8765824ca94eaefb8`. Preview and live marker/header/robots/deep-route checks passed. Signed-out Shop, Events, Calendar, and Account checks passed; Shop shows the approved static catalog, and Events/Calendar show only the fixed retry-later notice. Final control merge `cb6a8f0a418fc14b448bce5ded71d68520415c92` made the temporary manifest inactive; Netlify attempt `6a6dcdd47bc81e000859a249` stopped unpublished, exact-main CI run `30696264830` passed all five jobs, and provider plus public-marker readback left deploy `6a6dc9ea588b0c0008036312` live. Both temporary release-source refs are absent; exact rollback source `ed1b0833f25822cee80c99ded8753722b5608a3f` remains. This did not restore event records or deploy Firebase, Rules, Functions, provider configuration, account state, protected offers, officer editing, ledger, or production data. Keep rollback deploy `6a6dc219a8136300081811db` as dated incident evidence.
...
WEB-CONTENT-001 [#685](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/685) completed its website publication on 2026-09-14 Pacific time. Release ID `WEB-CONTENT-001-SEPTEMBER-LOCATIONS-2026-09-14` used [PR #686](https://github.com/Run-MPRC/Run-MPRC.github.io/pull/686), preview `6aa8c3e0e5faba00080fd632`, release-control head `f0c2d2378fd30b684ecb2fede447a184a414ab1c`, and all five successful jobs in PR CI run `34927419640`. Exact two-parent release merge `d25b0fe497e0179d62c060a2df3e3a7a9a2c68c6`, parents `925a677cd807dad1f925b4a85e89fc317b7a38b7` and `f0c2d2378fd30b684ecb2fede447a184a414ab1c`, tree `0d7452a7a3c95c43a65784724aea56f0d214477f`, passed all five exact-main jobs in run `34927937346`. Production deploy `6aa8c5b3fddd040009eba99c` became ready and published at `2026-09-15T04:13:59.373Z`. Its marker and all 62 served files matched source `41a9ae4ebc719212df610ff5cdcf9ca4eab18e71`, tree `7f75ebda7b9d295c5be26acc11b06b452dcc20c6`, previous source `7496fe0881fb52908c4ff2f40f488df09c94c908`, rollback deploy `6a7ece87c5ca4d0007c1a3fc`, and digest `cb4eba76c65c3501b37f9a0ad3cf00b6eefbc5a9c2e653f2b3c03607cfe64850`. The source differs from #659 only at `src/components/AnnouncementBanner.jsx` and `src/components/AnnouncementBanner.test.jsx`. Signed-out checks at 390×844 and 1280×900 confirmed exact notice text, both safe map links, banner and page-heading visibility, and no horizontal overflow; no screenshot was retained. [Repause PR #687](https://github.com/Run-MPRC/Run-MPRC.github.io/pull/687) used preview `6aa8c6b7e93356000806c950`, head `9b450d4a7ef118fd4cd114b87a7a9e079f949bb2`, tree `55150c633ed81cb2913c413c24c5733da5ef5733`, and all five successful jobs in PR CI run `34928224571`. Exact repause merge `609d7884e0242abce23449562e7e0da04794da1e`, parents `d25b0fe497e0179d62c060a2df3e3a7a9a2c68c6` and `9b450d4a7ef118fd4cd114b87a7a9e079f949bb2`, tree `55150c633ed81cb2913c413c24c5733da5ef5733`, passed all five exact-main jobs in run `34928449093`. Its Netlify attempt `6aa8c78dd7b9d1000952eaa2` errored unpublished with `published_at` null; provider and marker readback retained deploy `6aa8c5b3fddd040009eba99c`. The manifest is inactive. The temporary #685 source/control/repause refs are retired, while rollback ref `codex/netlify-source-685-rollback` remains pinned to #659 source `7496fe0881fb52908c4ff2f40f488df09c94c908` and deploy `6a7ece87c5ca4d0007c1a3fc`; #623 is older history. The banner shows both notices through Sept. 19, only Sept. 26 from Sept. 20 through Sept. 26, and none from Sept. 27 Pacific time. Email was already sent Sept. 12; WhatsApp still awaits the user's action-time confirmation, so issue #685 remains open. This release changed no Firebase, GitHub Pages, DNS, provider configuration, account, sign-in state, private data, payment, route, permission, or reusable release authority.

Remote-ref correction recorded 2026-09-14: the old #473/#623/#659 hot source refs remain present, superseding older absent/retired statements in this record. Inactive manifests block ordinary `main` publication, but an authorized manual rebuild of a historical active merge remains a latent manual-rebuild path. Do not call those old refs retired. Confirm each rollback ref, then use a separate approved cleanup before removing the corresponding hot ref.
...
COST-001 [#683](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/683) adds source-only resource limits for this pair: `minInstances: 0`, `maxInstances: 2`, `memory: '256MB'`, and `timeoutSeconds: 30`. The actual SDK endpoint metadata is tested. The backend verifier reads the v1 provider fields `minInstances`, `maxInstances`, `availableMemoryMb`, and `timeout`. The API defines minimum instances as a proto3 implicit-presence integer, so an omitted field represents zero; the summary records `minInstancesReadback: 'proto3-default-zero'` instead of claiming an explicit value was returned. Null, malformed or differing values and missing maximum/memory/timeout evidence are rejected. An unreadable or incomplete Function cannot pass through this normalization. Existing project/revision/Rules/trigger checks remain required. See the [API schema](https://github.com/googleapis/googleapis/blob/master/google/cloud/functions/v1/functions.proto) and [JSON presence semantics](https://protobuf.dev/programming-guides/json/#presence-and-default-values).

No migration is needed: profile schema, create-once transactions, claims, App Check/auth behavior, and trigger identities are unchanged. Execution timeouts and saturation may produce uncertain/busy results; the existing recovery path must preserve already committed data. Test cold starts and a small synthetic concurrent burst in staging before approving these initial limits for production. Max instances is not a hard monetary ceiling and may be briefly exceeded by the provider. Budgets/eligible spend caps, artifact retention, minimal runtime IAM, callable App Check enforcement, and the Node 20 maintenance deadline must be handled and verified in the provider release. Do not deploy extra Functions to make unrelated screens work. See the no-terminal [low-cost community review](./docs/officers/LOW_COST_COMMUNITY.md).
...
Source guard #674 may be reviewed before billing exists, but it is not deployment evidence. Stop before any provider mutation unless the board/platform owner separately approves a Cloud Billing account and spending controls. A billing link changes the project to pay-as-you-go even when expected Function usage is within no-cost quotas.
...
### Profile thumbnail and officer people finder — INERT FRONTEND PREVIEW; BACKEND NOT LIVE

Issue [#505](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/505) adds the private profile-photo/default-off-preference source slice of the optional people finder in [#504](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/504). Issue [#506](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/506) adds the minimum server-only projection, current-state reconciler, bounded officer name-search source, and separate gallery. Issue [#621](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/621) keeps that connected implementation behind one source-controlled availability boundary that defaults unavailable. Inspect its protected layouts only through synthetic local artifacts. If the exact #621 frontend is published, anonymous readback is limited to the revision, sign-in and administrator guards, and absence of a public member-directory request. Do not deploy or publish the connected #505/#506 behavior independently. None of these issues authorizes a real photo, real-name search, Firebase deployment, or production-data test.

#621 frontend-preview review must prove all of these without entering any name or choosing any file:

1. My Account shows a visible preview notice and the future thumbnail placeholder, file control, and independent finder choice are natively disabled.
2. The default account path does not mount the connected profile component, read a file or saved setting, create a request ID, or initialize or call a member-directory service.
3. `/admin/member-directory` remains behind `AdminGuard`; its name field and Search button are visibly described and natively disabled.
4. The default officer path accepts no name, creates no request ID, initializes no search service, and renders no sample, fake, or result card.
5. Preview copy uses future tense for backend behavior, says a future upload never opts a person in, says a future result is not membership proof, and states that there is no photo query or facial recognition.
6. Focused synthetic tests exercise the preserved connected branches through an explicit test seam; the production source default remains unavailable.
7. The disabled layouts remain readable at 320 pixels and expose useful accessible names, descriptions, status text, keyboard order, and contrast without horizontal overflow.

The preserved #505 connected-source review must prove all of these with generated non-face images and made-up accounts:

1. Missing preference returns hidden with revision zero.
2. Uploading one valid image creates only a bounded 256-pixel WebP thumbnail and does not turn on discoverability.
3. The submitted original, filename, metadata, and provider photo URL are never persisted or logged.
4. Replacing or removing a photo preserves the visibility choice; opting out preserves the private thumbnail and official account records.
5. An exact current command retry does not rewrite preference, photo, or audit state; a stale revision, different action or visibility value, or different normalized thumbnail under the same request ID cannot replace newer intent. Submitted files that normalize to the identical stored thumbnail have the same durable effect, so the server does not retain an original-file fingerprint. Each upload invocation still consumes one per-account rate-limit attempt before image processing, including an exact retry, so request-ID reuse cannot bypass the CPU-abuse bound.
6. Owners, members, and browser admins cannot directly read or write preference, photo, nested, or audit records.
7. Every mutation has one minimal server audit event without a name, photo, query, request body, or raw error.
8. My Account hides mutation controls after an uncertain result until the person deliberately reloads the private settings.
9. Every profile callable is declared with native App Check enforcement and fails closed unless its private response can carry explicit no-store headers.
10. If an opted-in person later clears their display name, My Account keeps opt-out available and explains that no officer result may include them until a valid name is restored; the #506 server response enforces that exclusion.

The preserved #506 connected-source review must also prove all of these with made-up names and generated non-face images:

1. A browser cannot directly read or write `memberDirectoryEntries`, preferences, photos, search audits, nested records, or rate state.
2. A missing, false, malformed, or later-disabled preference has no result; upload alone creates no searchable entry.
3. Opt-in, opt-out, name change, photo replace/remove, and a delayed member-write trigger all reconcile from current source records instead of trusting an old event payload.
4. A search transaction ordered after a completed opt-out conflicts/retries and returns no person; a stale or malformed projection is withheld. A search committed before opt-out is an earlier authorized snapshot and may arrive later, so the privacy notice must not promise retroactive recall.
5. Only native-App-Check callers with the exact verified `admin` compatibility role reach request parsing. Anonymous, unverified, member, malformed-claim, and App Check failures receive fixed denials.
6. The strict request is exactly one UUID and one explicit name query. Version-1 normalization uses NFKC, locale-independent lowercase, and Unicode letter/mark/number tokens; controls, format characters, unpaired surrogates, punctuation-only values, and normalized lengths outside 2–80 code units are rejected.
7. Search uses only domain-separated prefix digests, reads at most 48 candidates, exposes no total/cursor/pagination, and returns at most 24 exact `{entryRef, displayName, photo}` records. The entry reference is opaque and the photo is either null or the current fixed 256-pixel WebP projection.
8. The projection only selects candidates. The response name and photo are derived from current member, preference, and photo records inside the audited transaction and are checked against the current query again.
9. One audit contains only actor UID, fixed purpose/action, request ID, query-length bucket, result count, fixed outcome, and trusted time. It contains no query, entry reference, name, photo, source row, result identity, or raw error. A reused UUID is denied before candidate search and does not create a second audit or replay retained results.
10. The `/admin/member-directory` page sends nothing while typing, disables autocomplete, clears stale results when input or admin identity changes, handles loading/empty/fixed-failure/missing-photo states, and remains readable at 320 pixels.
11. `/admin/members` is visibly labeled as website-account administration: its broad name/email filter does not honor finder consent and is not a roster. It links to the separate People finder instead of being reused.
12. Search permits at most 30 attempts per verified-admin account in one hour. Rate state uses a separate domain-separated account pseudonym and the private TTL-dependent limiter. Prefix digests, entry references, rate keys, and audit identifiers are linkable pseudonyms, not anonymization, and none may enter browser logs, URLs, analytics, or screenshots.

Synthetic local artifacts and tests prove that the protected #621 layouts are disabled and their default branches make zero directory calls. Publishing and anonymously reading back the exact frontend proves only its revision, normal sign-in and administrator guards, and absence of a public member-directory request; it does not require or authorize a Firebase change. The connected release remains blocked on [#507](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/507), the #110 privacy decision, scoped authorization, protected short-lived deployment authority under #133, and isolated staging. When those dependencies are complete, deploy and verify the exact Rules plus `getMyMemberDirectoryProfile`, `setMyMemberDirectoryVisibility`, `setMyMemberDirectoryPhoto`, `removeMyMemberDirectoryPhoto`, `searchMemberDirectory`, and `syncMemberDirectoryEntryOnMemberWrite` while the frontend stays unavailable. Deploy and read back any required index with that backend set, and verify the trigger's retry policy. Before connected publication, prove that no older preference rows need projection or run an approved idempotent, dry-run-first current-state repair; retain that owned repair path for schema/normalization upgrades. Verify default-hidden, upload-without-opt-in, opt-in name search, opt-out absence, name change, stale-entry withholding, and photo removal with synthetic records only. Only after those gates pass may a separate reviewed source change turn frontend availability on and publish the connected website. A skipped or partial backend keeps the preview unavailable. Never test with a real member photo, inspect production profiles, widen Rules, or repair a record manually.

This feature performs no face recognition, image-to-person query, embedding, similarity scoring, or biometric template creation. A request for any of those is a stop condition and needs a separate owner/legal/privacy decision.
...
For OBS-001A3 [#142](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/142) and follow-up OBS-001A5 [#494](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/494), **source/tests** means application and public runtime source has no direct console path outside the closed four-outcome helper, the retired members-only literal is dropped, and synthetic console canaries pass; **merge** means the resulting `main` SHA; **website publication** and **`runmprc.com` verification** remain separate later evidence; **Firebase deployment** and **outside-provider configuration** are unchanged by this frontend-only source issue; **live behavior** and deletion of any earlier local browser-console history remain unverified. Never reproduce a member error or paste a browser console into an issue to prove this boundary.
