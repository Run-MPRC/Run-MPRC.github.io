...
| Staging | `run-mprc-staging` | No provider sandbox or backend test configuration yet | `run-mprc-staging.web.app`; `dev.runmprc.com` not configured | `staging`: two reviewers, exact `main`, read-only OIDC verifier | Static signed-out Hosting and protected project-identity proof are available; backend/provider staging remains a launch blocker |
...
`firebase.json` has a Hosting block for `build/` and missing-path SPA rewrites. Its predeploy chain rejects a Firebase CLI project that differs from the selected browser project, then rebuilds and scans executable JavaScript before upload. **Static engineering staging and a protected read-only authority check are available; backend and production publication are NOT AVAILABLE YET.** #665 used that chain for the first `run-mprc-staging` release. #667's WIF identity can read only the project identity; it cannot run this deploy chain. No protected production authority, custom domain, release marker, tested rollback, production release, or DNS/TLS change exists.
