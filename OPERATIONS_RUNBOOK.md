...
| Staging | `run-mprc-staging` | Empty Firestore Rules/index boundary plus an email/password-only Auth source contract that is not deployed; no Functions, App Check, or provider sandbox | `run-mprc-staging.web.app`; `dev.runmprc.com` not configured | `staging`: two reviewers, exact `main`, read-only OIDC verifier | Static signed-out Hosting, protected project-identity proof, empty delete-protected Firestore, and reviewed Auth source are available; usable backend/provider staging remains a launch blocker |
...
`firebase.json` has a Hosting block for `build/` and missing-path SPA rewrites. Its predeploy chain rejects a Firebase CLI project that differs from the selected browser project, then rebuilds and scans executable JavaScript before upload. It also has the #671 email/password-only Auth source block; that block is configuration intent, not provider evidence. **Static engineering staging, a protected read-only authority check, an empty Rules-protected Firestore boundary, and the reviewed Auth source contract are available; a usable backend and production publication are NOT AVAILABLE YET.** #665 used the Hosting chain for the first `run-mprc-staging` release. #667's WIF identity can read only the project identity; it cannot deploy. #669 was a separate bounded database/Rules/index operation and did not grant reusable authority. #671 provider deployment must wait for exact-main source and use its guarded Auth-only command. No protected Firebase deploy role, deployed Auth proof, Function, App Check enforcement, provider sandbox, production authority, custom domain, release marker, tested rollback, production release, or DNS/TLS change exists.
