...
The upgraded Rules helper fails before rule evaluation if Jest does not receive Node 20's native `fetch`, `Headers`, `Request`, `Response`, and `FormData`. The repository's dedicated Rules environment exposes those host APIs and no provider credential. The proof is 348/348 Rules cases, 71/71 focused Firebase/Auth/App Check tests, 198/198 frontend tests, 1,077/1,077 applicable Functions unit tests, 11/11 standalone callback cases, and 42/42 workflow/release safety cases. The optimized main JavaScript grows 8.25 kB gzip (about 3.3%); total build output grows 172 kB on disk (about 0.9%).
