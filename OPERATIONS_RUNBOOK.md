...
### Profile thumbnail and officer-finder preference — SOURCE ONLY, NOT LIVE

Issue [#505](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/505) adds only the first source slice of the optional people finder in [#504](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/504). Do not deploy or publish it independently. It has no officer search route, and it does not authorize real photos or production testing.
...
10. If an opted-in person later clears their display name, My Account keeps opt-out available and explains that no officer result may include them until a valid name is restored; the future #506 server response must enforce that exclusion.

The approved release order remains blocked on [#506](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/506), [#507](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/507), the #110 privacy decision, protected short-lived deployment authority, and isolated staging. When those dependencies are complete, deploy and verify the exact Rules and named profile/directory Functions before publishing the dependent website. A skipped or partial backend publishes nothing. Verify default-hidden, upload-without-opt-in, opt-in name search, opt-out absence, and photo removal with synthetic records only. Never test with a real member photo, inspect production profiles, widen Rules, or repair a record manually.
