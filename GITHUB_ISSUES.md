...
5. [#459](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/459): update only the compatible brace-expansion 1.x and 5.x lock records, pin every installed family, and keep the separate minimatch migration explicit.
...
- SUPPLY-001D10/#459: keep `package.json` byte-identical while moving twelve brace-expansion 1.x records at 1.1.11, 1.1.12, or 1.1.14 to 1.1.16 and the sole 5.x record from 5.0.5 to 5.0.7; the two 2.x records remain 2.1.2. Exactly 13 package records change, and only their version, public-registry URL, and integrity fields move. A dependency-free RED test pins all 15 locked/installed paths, development classifications, parent ranges, and identities. In bounded child processes, the old 1.x line exhausts its stack on the maintained 5,000-group regression and the old 5.x line exceeds one second on the maintained 30-group regression; both patched lines finish with unchanged ordinary sequence/options behavior. A clean Node 20 full audit changes from 58 findings (24 high) to 57 (23 high), and the production-only audit changes from 2 high and 1 low to 1 high and 1 low. Clean baseline and patched builds are byte-for-byte identical across all 59 files and 19,520,717 bytes. Functions manifests, lockfile, and audits remain unchanged. The remaining minimatch high group is explicitly unresolved: the audit service's legacy patch labels are not functional proof, and a separately reviewed modern-major compatibility migration is required.
