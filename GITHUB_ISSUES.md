...
5. Upgrade Firebase Admin/Functions with emulator and Auth/Firestore trigger tests.
6. Upgrade Stripe SDK and deliberately select/test the Stripe API version.
7. Plan and execute CRA migration to a maintained build tool without mixing payment behavior changes.
8. Add automated dependency updates, audit policy, reviewed lockfiles, and an SBOM/release inventory.
...
Never use `npm audit fix --force`. Do not combine all upgrades in one unreviewable lockfile diff. SUPPLY-001A, B, C, focused D8, and CLI/Java D9 are represented by #127, #192, #202, #453, and #455; do not publish duplicates. #202, #453, and #455 are source/test evidence only: they do not publish the website, deploy Firebase, change provider settings, or prove live behavior. The remaining Babel-core, brace-expansion, minimatch, other CLI-transitive, Firebase Admin/Functions, and Functions-lock findings are unresolved and not accepted as safe. Stop if a later provider SDK change needs a product/runtime decision.
