(function initializeSpaNavigation(root, factory) {
  const api = factory();
  ...
  function parseSameOriginTarget(storedTarget, origin) {
    ...
    try {
      const target = new URL(storedTarget, origin);
  ...
  function restoreRedirect(browserWindow) {
    ...
    const target = parseSameOriginTarget(storedTarget, browserWindow.location.origin);
