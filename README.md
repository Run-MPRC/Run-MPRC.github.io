...
**Deployment reality checked 2026-08-01:** merges run CI but do not start the manual release workflow. The protected gate accepts one exact current merged commit, rechecks its newest CI run after approval, uses one fixed Firebase target set, fails when protected authority/configuration is missing, verifies Firebase before publishing GitHub Pages, and gives no server credential to website preparation or publication. Ordinary Git-triggered Netlify production builds are paused. A too-broad #473 web artifact published as deploy `6a6dc0167fbe68000816b448`, was restored through merge `1099ee8e6fdb81141fd9460de175b6d854cbcfdd` to deploy `6a6dc219a8136300081811db`, and was re-paused by merge `dee79511b6e371329aa129139729e112e7a51aad`; the re-pause attempt did not publish. The live public marker still proves source `ed1b0833f25822cee80c99ded8753722b5608a3f`. That restored artifact still exposes raw Shop/Events permission failures, and #473's narrower replacement is **NOT AVAILABLE YET**. None of these web-only controls deploy Firebase. The source stops adding a Pages `CNAME`, but GitHub Pages currently still claims `runmprc.com` and its default URL redirects there; only a controlled #136/WEB-001 publication and provider readback can clear that conflict. Reusable protected publication to the live Netlify-served `runmprc.com` is not configured yet. Treat GitHub Pages, Netlify, `runmprc.com`, Firebase, and outside providers as separate states.
