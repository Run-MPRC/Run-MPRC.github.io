...
| Events and activities | Firestore event records, `src/pages/events/`, `src/pages/activities/Activities.jsx`, and `src/images/activities/` | Public visitors see the club introduction above the schedule. Signed-in users see the schedule without the introductory pitch. Admin saves can affect production data immediately; live behavior is not verified. |
| Shop and discounts | Firestore product records, `src/pages/shop/`, and `src/text/externalLinks.js` | The discounts page is an outside public link. Live commerce is not approved. |
...
| Contact and social media | `src/text/ContactUs.js`, `src/pages/contact/Contact.jsx`, `src/text/externalLinks.js`, and `src/images/contact/` | Some text and email parts are hardcoded. |
