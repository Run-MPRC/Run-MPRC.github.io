...
**Purpose:** allow only the first valid race or Shop Checkout submission until the visitor leaves or reloads the page, including repeats made before the busy label appears.
...
```mermaid
flowchart TD
    A["Made-up race or Shop form passes its current checks"] --> B{"Has this page visit already sent a Checkout request?"}
    B -- "Yes" --> C["Ignore the repeated submission"]
    B -- "No" --> D["Immediately record that one request started"]
    ...
    C --> I["No second request or race submission-count record"]
    H --> J["Stop; do not retry on this page"]
```

Text alternative: after the current page checks pass, the page immediately records that one Checkout request has started. It does this before using the test replacement or showing the busy label. Another submission is ignored until the visitor leaves or reloads the page. The first result either follows the existing success path or uses #357's fixed unknown-result stop. The page makes no second request, but a reload, another tab or device, a script, or another program that calls the website connection directly remains outside this guard.
...
4. Confirm two immediate valid race submissions produce one Checkout request.
5. Confirm those race submissions produce one analytics record, which is the site's count of submission attempts.
6. Confirm two immediate valid Shop submissions produce one Checkout request.
7. Confirm the test report shows that the first request is recorded before the busy label appears or the request finishes.
8. Confirm the existing submit button shows its disabled busy label while the first request is unfinished.
9. Confirm the other form controls remain editable. This change does not lock the complete form.
10. Confirm a race form with an unaccepted waiver sends no request.
11. Confirm the same made-up race form can send one request after the waiver is accepted.
12. Confirm a Shop button disabled by the current required-field checks sends no request when clicked.
13. Confirm one made-up free participant reaches the existing success page after one request.
14. Confirm one made-up volunteer reaches the existing success page after one request.
15. Confirm the volunteer request omits the participant price choice.
16. Confirm one made-up accepted race paid link is assigned once after one request.
17. Confirm one made-up accepted Shop paid link is assigned once after one request.
18. Confirm the race paid link is test output.
19. Confirm the Shop paid link is test output.
20. Confirm the page keeps its first-request record until the visitor leaves or reloads the page.
21. Confirm a rejected request or missing paid link uses #357's fixed unknown-result stop.
22. Confirm moving directly to another event or item address on the same open screen keeps Checkout locked.
23. Stop and ask for a separate fix if an older unfinished result could affect that new address.
24. Confirm the tests use no real contact, member, order, event, Firebase, Stripe, or payment data.
25. Record the source change result.
26. Record the test result.
27. Record the merge result.
28. Record the website publication result.
29. Record the `runmprc.com` result.
30. Record the Firebase deployment result.
31. Record the Stripe result.
32. Record the outside-provider result.
33. Record the production-data result.
34. Record the checkout result.
35. Record the payment result.
36. Record the live-behavior result.

**Expected result:** one race or Shop page visit sends at most one Checkout request after the current page checks pass. Immediate repeats and repeats after the busy label appears are ignored. A failed page check does not use up the one request. This page-only protection does not stop duplicate requests at the server, prove that Stripe acted, prove payment, or match Stripe records to website records.

**Stop conditions:** a second Checkout request or race analytics record from the same made-up pair; a valid first request blocked by an earlier unaccepted waiver or disabled Shop button; loss of the first-request record before the visitor leaves or reloads the page; any real person, event, item, Session, checkout link, payment, Firebase record, Stripe call, production test, or provider detail; an instruction to retry after an unknown result; or a claim that source, tests, merge, preview, or green CI makes checkout safe or live.

**Success proof:** exact #503 pull request and merge commit; recorded old-source two-request failures followed by green immediate-repeat, page-check, free-participant, volunteer, race-link, Shop-link, and after-busy-label tests; unchanged reviewed lint counts; relevant full checks; independent security, payment-lifecycle, and backup-officer reviews; and an explicit statement that no website, `runmprc.com`, Firebase, Stripe/provider, production-data, checkout, payment, or live behavior was changed or verified.
...
No system-topology map changes are required. This source change adds one short-lived page record and changes no account, permission, data store, server, provider, or publishing boundary.
