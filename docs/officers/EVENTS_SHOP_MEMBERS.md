...
The public Events page now combines the former Activities introduction with the current/upcoming event list. Signed-in people see the event list without the introductory pitch. The month calendar reads the same event records. Admin source screens can create and edit those records, but the screens remain **NOT AVAILABLE YET** as an officer continuity procedure until the safeguards below are proven.

The Shop page links to the public MPRC discounts site. That link does not make discount eligibility, provider terms, or live checkout behavior authoritative in this application.
...
## Signed-in waiver enforcement — BLOCKED

The site does not yet have trustworthy account-level proof that every signed-in person accepted an approved waiver. The embedded Google Form and a browser `localStorage` checkbox cannot prove who signed, which waiver version they accepted, or whether the evidence is immutable.

Before enforcement can be enabled, the club officer and approved legal/privacy owner must approve the waiver version, canonical text or PDF, effective date, minor/guardian rules, evidence fields, retention period, and who may correct an error. Then DATA-002 must add server-owned evidence, deny unsigned accounts at protected server and Firestore boundaries, test bypasses and version changes, and provide an officer-safe exception/escalation procedure. Do not treat a redirect or checkbox as proof.

## Account profile failures

The Account page may let a signed-in person update only their own display name and phone number. It must not expose raw Firebase or provider errors. If the profile cannot be loaded securely, the page shows a retry action and disables editing instead of presenting an empty form. This safer error handling exists in source and tests; it does not prove current production Rules or Functions are deployed.

If a member reports “Account details are unavailable,” record only the time, affected page, and their Firebase account UID through the approved private support path. Do not ask them to paste profile data, tokens, screenshots containing personal details, or raw production records into an issue or AI tool. Escalate to the platform owner to compare website, Firestore Rules, Auth, Functions, and App Check deployment evidence.
