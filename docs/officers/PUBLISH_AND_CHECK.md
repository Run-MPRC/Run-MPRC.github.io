...
**Firebase staging status:** **STATIC HOSTING, EMPTY FIRESTORE, DISPOSABLE EMAIL/PASSWORD AUTH PROOF, AND APP CHECK ENFORCEMENT FOR AUTHENTICATION PLUS FIRESTORE ARE AVAILABLE; FUNCTIONS, OFFICER OPERATION, AND PRODUCTION RELEASE ARE NOT AVAILABLE YET.** WEB-001A1 [#663](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/663) requires controlled optimized builds to name staging or production and scans executable JavaScript for environment identity. WEB-001A2 [#665](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/665) created the club-owned `run-mprc-staging` project. CI-001D1 [#669](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/669) adds an empty delete-protected Firestore database and exact reviewed Rules/indexes; fixed anonymous server-only reads were denied and the root collection count remained zero. CI-001D2 [#671](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/671) deploys password-required email/password Auth from exact merged source, verifies provider/privacy/domain settings, passes disposable API/browser sign-in checks, and returns the project to zero users and records. CI-001D3 [#674](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/674) adds only a fail-closed code guard for the future paired profile Functions; billing remains unapproved and no Function is live. CI-001D4 [#676](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/676) merged its separate staging Hosting guard through [PR #677](https://github.com/Run-MPRC/Run-MPRC.github.io/pull/677), then configured one score-based App Check key restricted to the two staging hosts, one matching web-app registration with one-hour tokens, and exact source `bce911a7083201cfe2141edaa9660f210287bf57` as Hosting version `8556fc51210bdc66`. Authentication plus Firestore now read `ENFORCED` with replay protection `OFF`. A made-up browser account passed create, sign-out, fresh sign-in, and post-enforcement sign-in; signed-in and signed-out empty Events reads passed; tokenless Auth and Firestore requests were denied; and cleanup returned users plus root collections to zero. Officers do not sign in, enter information, run commands, configure App Check, attach billing, or use the site as a publish control. Provider sandboxes, protected short-lived publication, release marker, tested rollback, complete security headers, custom domain, and DNS/TLS change remain **NOT AVAILABLE YET**. Billing is disabled, no Functions-related API or Function exists, the account profile fails closed while Functions are absent, production Firebase Hosting remains empty, and Netlify still serves `runmprc.com`.

**Live Netlify publication status:** a reusable protected release is **NOT AVAILABLE YET**. Ordinary Git-triggered production builds are paused by repository configuration. WEB-CONTENT-001 [#685](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/685) is production as exact deploy `6aa8c5b3fddd040009eba99c` from source `41a9ae4ebc719212df610ff5cdcf9ca4eab18e71`. Its marker and all 62 files matched, signed-out phone/desktop notice checks passed, and repause attempt `6aa8c78dd7b9d1000952eaa2` published nothing. The manifest is inactive, temporary #685 source/control/repause refs are absent, and rollback ref `codex/netlify-source-685-rollback` remains. #659 deploy `6a7ece87c5ca4d0007c1a3fc` and source `7496fe0881fb52908c4ff2f40f488df09c94c908` are immediate rollback; #623 deploy `6a7e072f8f346b0008510d29` and #473 deploy `6a6dc9ea588b0c0008036312` are older history. The old #473/#623/#659 hot source refs remain a documented latent manual-rebuild risk despite inactive manifests. Shop is the static catalog; Events and Calendar show a fixed retry-later notice; the directory remains inert. Event records remain unavailable because this did not deploy Firebase. GitHub Pages currently still claims the same custom domain; future source omits that claim, but #136/WEB-001 must publish and verify its removal.

**Completed #659 exception:** WEB-002D [#659](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/659) completed one exact-artifact accessibility release on 2026-08-14. The frozen source contains only reviewed #291 visible focus, #490 phone-menu disclosure/close behavior, and #657 client-side route focus. Deploy `6a7ece87c5ca4d0007c1a3fc` was production after that release and is now the immediate rollback for #685; #623 deploy `6a7e072f8f346b0008510d29` was its rollback target. This is not an officer-operated or reusable control. The no-terminal section below is retained as the historical audit record.

## Temporary #685 meeting-location release — WEBSITE COMPLETED 2026-09-14
...
**Completed evidence:** release ID `WEB-CONTENT-001-SEPTEMBER-LOCATIONS-2026-09-14`; issue [#685](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/685); [release PR #686](https://github.com/Run-MPRC/Run-MPRC.github.io/pull/686); exact approved public wording and map links; frozen source `41a9ae4ebc719212df610ff5cdcf9ca4eab18e71`; tree `7f75ebda7b9d295c5be26acc11b06b452dcc20c6`; 62-file digest `cb4eba76c65c3501b37f9a0ad3cf00b6eefbc5a9c2e653f2b3c03607cfe64850`; expected first parent `925a677cd807dad1f925b4a85e89fc317b7a38b7`; immediate rollback deploy `6a7ece87c5ca4d0007c1a3fc` and source `7496fe0881fb52908c4ff2f40f488df09c94c908`; retained rollback ref `codex/netlify-source-685-rollback`; all required checks; the signed-out pinned preview; and the completed manifest-disable change.

```mermaid
flowchart LR
    Live["#659 deploy was live"] --> Preview["Exact #685 frozen preview passed"]
    ...
    Marker -- "Yes" --> Production["#685 deploy verified live"]
    Production --> Repause["Exact disable change merged"]
    Repause --> Quiet["Manifest inactive; later merges publish nothing"]
```

In words: #659 stayed live while the exact #685 preview was reviewed. The unchanged recorded `main` parent merged the control. The public marker, every file, and the banner matched before the exact disable change made ordinary publication inactive again. #685 remains live; #659 is immediate rollback.
...
**Success proof:** [PR #686](https://github.com/Run-MPRC/Run-MPRC.github.io/pull/686) head `f0c2d2378fd30b684ecb2fede447a184a414ab1c` used preview `6aa8c3e0e5faba00080fd632` and passed all five jobs in run `34927419640`. Exact release merge `d25b0fe497e0179d62c060a2df3e3a7a9a2c68c6` had parents `925a677cd807dad1f925b4a85e89fc317b7a38b7` and `f0c2d2378fd30b684ecb2fede447a184a414ab1c`, tree `0d7452a7a3c95c43a65784724aea56f0d214477f`, and passed all five exact-main jobs in run `34927937346`. Production deploy `6aa8c5b3fddd040009eba99c` became ready and published at `2026-09-15T04:13:59.373Z`. Its marker and all 62 served files matched source `41a9ae4ebc719212df610ff5cdcf9ca4eab18e71`, tree `7f75ebda7b9d295c5be26acc11b06b452dcc20c6`, previous source `7496fe0881fb52908c4ff2f40f488df09c94c908`, rollback deploy `6a7ece87c5ca4d0007c1a3fc`, and digest `cb4eba76c65c3501b37f9a0ad3cf00b6eefbc5a9c2e653f2b3c03607cfe64850`. Signed-out checks at 390×844 and 1280×900 confirmed the exact notice text, both safe map links, banner and page-heading visibility, and no horizontal overflow; no screenshot was retained. [Repause PR #687](https://github.com/Run-MPRC/Run-MPRC.github.io/pull/687) used preview `6aa8c6b7e93356000806c950`, head `9b450d4a7ef118fd4cd114b87a7a9e079f949bb2`, tree `55150c633ed81cb2913c413c24c5733da5ef5733`, and passed all five jobs in run `34928224571`. Exact repause merge `609d7884e0242abce23449562e7e0da04794da1e` had parents `d25b0fe497e0179d62c060a2df3e3a7a9a2c68c6` and `9b450d4a7ef118fd4cd114b87a7a9e079f949bb2`, tree `55150c633ed81cb2913c413c24c5733da5ef5733`, and passed all five exact-main jobs in run `34928449093`. Attempt `6aa8c78dd7b9d1000952eaa2` errored unpublished with `published_at` null and retained #685 deploy `6aa8c5b3fddd040009eba99c`. The manifest is inactive; temporary #685 source/control/repause refs are absent; rollback ref remains. Firebase, Pages, DNS, providers, accounts, private data, payments, routes, and permissions were unchanged.
...
#685's website work is complete, and production is deploy `6aa8c5b3fddd040009eba99c`. Both notices show through Sept. 19, only Sept. 26 shows from Sept. 20 through Sept. 26, and the banner is absent from Sept. 27 Pacific time. Email was already sent Sept. 12. WhatsApp remains pending the user's action-time confirmation, so issue #685 remains open.

Remote readback on 2026-09-14 found the old #473/#623/#659 hot source refs still present; this supersedes older absent/retired wording in this guide. Their manifests are inactive, so ordinary merges remain paused, but a latent manual-rebuild path remains. Officers do not remove those old refs. The platform owner must confirm the rollback refs and use a separate approved cleanup.
...
- #676 adds a separate argument-closed source wrapper for Hosting only. It requires exact staging identity, the club account, short-lived token path, pinned CLI, and one bounded Enterprise public key in the executable artifact. Provider and browser evidence now proves one restricted key, exact source `bce911a7083201cfe2141edaa9660f210287bf57` as Hosting version `8556fc51210bdc66`, Auth plus Firestore `ENFORCED` with replay protection `OFF`, tokenless denial, token-bearing browser success, and cleanup to zero.
...
- The club-owned `run-mprc-staging` project supplies one signed-out Hosting surface, one empty Firestore boundary, narrow email/password Auth, and enforced App Check for Auth plus Firestore. Current Hosting is exact source `bce911a7083201cfe2141edaa9660f210287bf57`, version `8556fc51210bdc66`; direct `/events`, disposable Auth, tokenless denial, token-bearing browser behavior, and zero-count cleanup passed. Its Rules/index release comes from exact source `ee16bd16220ab58bd3a2add80dd2f39a1d514dd7`; the 418-case emulator suite, provider definition match, fixed anonymous denial probes, and zero-root-collection readback passed.
- #663 source requires complete staging-named public browser configuration and gives CI a synthetic staging artifact scan. #665 proves the staging host. #667 adds a protected read-only identity check, not Firebase deployment authority. #669 proves the empty Firestore Rules/index boundary. #671 proves the narrow Auth configuration and one earlier disposable lifecycle. #676 proves provider registration, exact Hosting publication, `ENFORCED`/replay-`OFF` readback, tokenless denial, token-bearing browser success, and cleanup. These slices do not supply Functions, outside-provider isolation, a release marker, predecessor rollback, or permission to test real/private/admin/commerce behavior.
...
- Ordinary Git-triggered Netlify production builds are paused. The completed #473, #623, and #659 exceptions each used a bounded exact-parent release, but their old hot source refs remain even though their manifests are inactive; this is a latent manual-rebuild risk. #659 deploy `6a7ece87c5ca4d0007c1a3fc` is now the immediate rollback for current #685 production. The temporary #685 source/control/repause refs are retired, and rollback ref `codex/netlify-source-685-rollback` remains pinned to exact #659 source `7496fe0881fb52908c4ff2f40f488df09c94c908`.
...
## Review staging App Check enforcement — AVAILABLE AFTER #676

**Purpose:** let a backup officer confirm the current #676 provider, Hosting, enforcement, denial, browser, and cleanup evidence without signing in, running a command, or handling a key or credential.

**Approver:** platform owner plus the named security reviewer for #676.

**Prerequisites:** issue #676 and its completion evidence; merged source PR #677; exact merge `bce911a7083201cfe2141edaa9660f210287bf57`; all five successful jobs in exact-main CI run `33018136439`; and the redacted final provider/browser record.
...
flowchart LR
    ...
    Artifact --> Provider["One score key; two staging hosts;\none-hour web-app registration"]
    Provider --> Hosting["Exact source bce911a7\nHosting version 8556fc51210bdc66"]
    Provider --> Enforce["Auth + Firestore ENFORCED;\nreplay protection OFF"]
    Hosting --> Public["Root and Events 200; empty UI;\nno browser warning or error"]
    Enforce --> Deny["Tokenless Auth 401;\ntokenless Firestore 403"]
    Enforce --> Browser["Disposable create/sign-out/sign-in;\ntoken-bearing Events passed"]
    Browser --> Cleanup["Identity deleted; users and\nroot collections zero"]
    Cleanup -. "Functions and private flows absent" .-> Pending["Do not call complete backend"]
    Broad["Branch deploy, broad scope, debug token,\nbilling, production, or real data"] --> Stop["Stop"]
```

Text alternative: exact merged source passed the staging-only guard, one restricted provider key was published in one exact Hosting version, Auth plus Firestore enforce App Check, tokenless requests were denied, genuine browser requests passed, and cleanup returned both counts to zero; Functions, billing, production, broad scope, debug tokens, and real data still stop.

1. Open issue [#676](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/676) and source [PR #677](https://github.com/Run-MPRC/Run-MPRC.github.io/pull/677).
...
6. Confirm the made-up browser identity completed create, sign-out, fresh sign-in, and post-enforcement sign-in, and that signed-in plus signed-out empty Events reads succeeded without warnings/errors.
7. Confirm tokenless Auth returned HTTP 401 and tokenless Firestore returned HTTP 403 while the genuine browser path continued to work.
8. Confirm Authentication and Firestore read `ENFORCED`, replay protection reads `OFF`, users and root collections are zero, billing is disabled, and no Functions-related API or Function exists.
9. Confirm production Firebase Hosting, `runmprc.com`, Netlify, DNS, and production data are unchanged.
10. Record the issue, pull request, exact merge, CI run, Hosting version, approver, and check date. Do not paste a key, provider locator, token, account response, user identifier, or private screenshot.

**Expected result:** a backup officer can verify that Auth and Firestore enforce App Check in engineering staging while correctly reporting that Functions, private flows, and production are not available yet.

**Stop conditions:** stop if the source or evidence names another account/project, permits more than Hosting, asks for a credential/debug token, reveals a key or user identifier, links billing, changes production, uses real data, differs from the exact provider/Hosting/count readback, or claims Functions/private behavior from App Check evidence.

**Success proof:** keep #676, PR #677, exact merge, exact-main CI link, source-test summary, redacted provider settings, Hosting version, disposable browser lifecycle, tokenless denial, token-bearing Events result, zero-count/billing/Functions readback, named approver, and check date.

**Undo:** stop staging sign-in use and open one urgent reviewed issue for the platform/security owners. Only if they approve a rollback may the platform maintainer atomically return both Authentication and Firestore to `UNENFORCED`, verify that readback, and preserve the restricted provider configuration for diagnosis. Do not ask an officer to edit provider settings or delete the project.

**Escalation:** platform owner first, security owner second. Use the private incident path for credential-shaped output, a key/user identifier in public evidence, or an unexpected provider, account, project, deployment, billing, user, record, or enforcement change.
...
**Prerequisites:** the exact approved commit must be live and identified by the host; the public site must be safe to open without signing in; and the observer must use a normal computer with a keyboard. The reusable protected website release is still **NOT AVAILABLE YET** under #133/#136. The current host marker must identify #685 deploy `6aa8c5b3fddd040009eba99c` and exact source `41a9ae4ebc719212df610ff5cdcf9ca4eab18e71`. That artifact retains the route-focus behavior first published by #659. The completed #659 route-focus audit did not separately retain a keyboard `:focus-visible` observation or screenshot; repeat the safe steps below if that additional evidence is required. #659 deploy `6a7ece87c5ca4d0007c1a3fc` is the immediate rollback host record; #623 is older history.
...
**Prerequisites:** issue #657; reviewed PR #658 and exact head `a411cb4ebcfb4f1f05b3883721aa55f3d72bc701`; the recorded old-source failure; the named green WEB-UX-004 and full App results; and a reviewed undo. The source check uses GitHub and test summaries only. It needs no terminal, account, sign-in, private page, or real data. #659 first published the equivalent route-focus behavior with its #291/#490 prerequisites in source `7496fe0881fb52908c4ff2f40f488df09c94c908` as deploy `6a7ece87c5ca4d0007c1a3fc`; signed-out desktop and phone route-focus/menu checks passed. Current #685 production retains that behavior as deploy `6aa8c5b3fddd040009eba99c`, and its exact marker must identify source `41a9ae4ebc719212df610ff5cdcf9ca4eab18e71`. Reusable protected publication remains **NOT AVAILABLE YET** under #133/#136. #659 is immediate rollback; #623 Netlify deploy `6a7e072f8f346b0008510d29` and source `c2d87d1f69f15e128a0bc9b1b9f915b7c8417aec` are older history.
