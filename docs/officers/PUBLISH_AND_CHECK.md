...
- #676 adds a separate argument-closed source wrapper for Hosting only. It requires exact staging identity, the club account, short-lived token path, pinned CLI, and one bounded Enterprise public key in the executable artifact. The provider checkpoint has now registered one restricted key, published exact source `bce911a7083201cfe2141edaa9660f210287bf57` as Hosting version `8556fc51210bdc66`, and placed Auth plus Firestore in monitoring mode. That checkpoint is not `ENFORCED` readback, missing-token denial, or final token-bearing browser evidence.
...
- The club-owned `run-mprc-staging` project supplies one signed-out Hosting surface, one empty Firestore boundary, narrow email/password Auth, and an App Check monitoring checkpoint. Current Hosting is exact source `bce911a7083201cfe2141edaa9660f210287bf57`, version `8556fc51210bdc66`; direct `/events`, empty Firestore/UI, and browser warning/error checks passed. Its Rules/index release comes from exact source `ee16bd16220ab58bd3a2add80dd2f39a1d514dd7`; the 418-case emulator suite, provider definition match, fixed anonymous denial probes, and zero-root-collection readback passed.
- #663 source requires complete staging-named public browser configuration and gives CI a synthetic staging artifact scan. #665 proves the staging host. #667 adds a protected read-only identity check, not Firebase deployment authority. #669 proves the empty Firestore Rules/index boundary. #671 proves the narrow Auth configuration and one earlier disposable lifecycle. #676 now proves provider registration, exact Hosting publication, and monitoring settings only. These slices do not supply Functions, App Check enforcement/denial/replay evidence, outside-provider isolation, a release marker, predecessor rollback, or permission to test real/private/admin/commerce behavior.
...
## Review the staging App Check monitoring checkpoint — AVAILABLE; ENFORCEMENT NOT AVAILABLE YET

**Purpose:** let a backup officer confirm the current #676 provider and Hosting checkpoint, and distinguish monitoring from enforcement, without signing in, running a command, or handling a key or credential.

**Approver:** platform owner plus the named security reviewer for #676.

**Prerequisites:** issue #676; merged source PR #677; exact merge `bce911a7083201cfe2141edaa9660f210287bf57`; all five successful jobs in exact-main CI run `33018136439`; the redacted provider checkpoint; and an issue statement that enforcement/browser completion is still pending.
...
flowchart LR
    ...
    Provider --> Monitor["Auth + Firestore UNENFORCED;\nreplay protection off"]
    Hosting --> Public["Root and Events 200; empty UI;\nno browser warning or error"]
    Monitor -. "Disposable Auth, ENFORCED,\nmissing-token and replay proof pending" .-> Pending["Do not call protected yet"]
    Broad["Branch deploy, broad scope, debug token,\nbilling, production, or real data"] --> Stop["Stop"]
```

Text alternative: exact merged source passed the staging-only guard, one restricted provider key was registered and published in one exact Hosting version, and Auth plus Firestore now monitor requests; enforcement, denial, and the new disposable browser Auth proof remain pending, while broad scope, debug tokens, billing, production, and real data stop.
...
**Expected result:** a backup officer can verify that the provider and exact Hosting revision are live in monitoring mode while correctly reporting that App Check enforcement and the new browser Auth/denial/replay proof are not available yet.

**Stop conditions:** stop if the source or evidence names another account/project, permits more than Hosting, asks for a credential/debug token, reveals a key or user identifier, links billing, changes production, uses real data, differs from the exact provider/Hosting/count readback, or calls monitoring enforced/protected.

**Success proof:** keep #676, PR #677, exact merge, exact-main CI link, source-test summary, redacted provider settings, Hosting version, public browser result, zero-count/billing/Functions readback, named approver, check date, and explicit enforcement/browser states still pending.

**Undo:** while services remain `UNENFORCED`, stop staging sign-in use and open one reviewed issue for the platform/security owners. If a later enforcement attempt breaks the browser, the platform maintainer atomically returns both Authentication and Firestore to `UNENFORCED`, verifies that readback, and preserves the restricted provider configuration for diagnosis. Do not ask an officer to edit provider settings or delete the project.
