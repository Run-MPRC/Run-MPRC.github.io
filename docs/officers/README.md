...
The empty staging Firestore boundary is available after #669. Project `run-mprc-staging` has a delete-protected empty database with the exact reviewed Rules and indexes. Fixed signed-out requests to server-only paths were denied, and the final root collection count was zero. This is not a usable backend: Auth test setup, Functions, App Check, provider sandboxes, reusable deployment authority, and signed-in testing remain **NOT AVAILABLE YET**. Officers do not sign in, enter records, run commands, or edit the database. Use the no-terminal [staging Firestore review](./PUBLISH_AND_CHECK.md#review-the-empty-staging-firestore-boundary--available-after-669).
