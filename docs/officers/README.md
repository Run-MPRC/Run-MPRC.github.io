...
The empty staging Firestore boundary is available after #669. Project `run-mprc-staging` has a delete-protected empty database with the exact reviewed Rules and indexes. Fixed signed-out requests to server-only paths were denied, and the final root collection count was zero. #671 first adds only a reviewed email/password Auth source contract and a staging-only deployment guard; it does not initialize Auth or create a user. This is not a usable backend: deployed Auth proof, Functions, App Check, provider sandboxes, reusable deployment authority, and signed-in testing remain **NOT AVAILABLE YET**. Officers do not sign in, enter records, run commands, edit the database, or deploy Auth. Use the no-terminal [staging Firestore review](./PUBLISH_AND_CHECK.md#review-the-empty-staging-firestore-boundary--available-after-669) and [Auth source review](./PUBLISH_AND_CHECK.md#review-the-staging-auth-source-contract--source-only-not-deployed).
