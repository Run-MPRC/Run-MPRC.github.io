...
Independent officer publishing to the live Netlify host is **NOT AVAILABLE YET**. An overbroad #473 artifact was published and rolled back on 2026-08-01. The bounded replacement remains live as deploy `6a6dc9ea588b0c0008036312` from source `39ab8649df411262c8109a3c81a57bc38f1e168b`: Shop is the static pickup catalog, and Events/Calendar show a fixed retry-later notice instead of a raw provider error. Event records remain unavailable. #623 is one separate, exact-artifact release under review for the inert member-directory interface; it has not published or been verified, and the #473 deploy is its rollback target. This is not a reusable officer control. Use a platform maintainer until the Netlify connection and rollback path are documented and tested.
...
The optional profile-photo and officer People-finder functions are **NOT AVAILABLE YET**. #621 makes the frontend source default an inert preview. My Account shows the future photo and independent finder-choice controls disabled. The administrator-guarded People finder shows its name field and Search button disabled. Neither preview reads saved state, accepts a file or name, creates a request, calls the directory services, or shows people. #623 freezes only that inert interface for one exact Netlify release, but it remains under review and is not production. Officers inspect the protected disabled layouts only in synthetic local artifacts. If publication later passes, signed-out public review proves only the exact revision, normal sign-in and administrator guards, and absence of a member-directory network request. Follow [Events, shop, members, and money](./EVENTS_SHOP_MEMBERS.md), and do not sign in to production or use a real name or photo. #623 changes no Firebase, provider, account, sign-in, or production data. #507 must later prove privacy approval, scoped authorization, isolated staging, backend-first deployment and readback, and a separate reviewed source flip before connected use. The future connected feature is not the official roster and performs no photo query or facial recognition.
