...
The former workflow automatically published Pages before attempting Firebase and could finish green after skipping Firebase. #135 replaces that source path with a manual exact-current-commit request, exact latest CI checks, one fixed backend plan, protected short-lived identity wiring, provider readback, and Firebase-before-Pages publication. Missing authority or failed/partial verification is red. Ordinary Git-triggered Netlify production builds stop. Replacement #473 continued only for one production `main` merge whose first parent was exact `dee79511b6e371329aa129139729e112e7a51aad`, then independently fetched and verified source commit `39ab8649df411262c8109a3c81a57bc38f1e168b` and tree `d76b496cdc5e79015bc048cb961758abbe88b9ce` before matching the fixed 60-file digest. Review also compared executable code against live artifact source `ed1b0833`; provenance alone did not authorize accumulated source. Its preview and production artifacts matched, and the manifest was then made inactive.

#623 now proposes the same fail-closed pattern for one inert member-directory interface artifact. The reviewed input must remain exact source `c2d87d1f69f15e128a0bc9b1b9f915b7c8417aec`, tree `411aa6ec9a9459f5d923030533ffc7c007fe6908`, 62 files, digest `d837272a1e5efc1575809e87f532276b38d1a63f1dd79ec1aef0533f6da8afb1`, and first parent `019353361210021483f23003e09ee6924b78e67c`. Current production remains #473 deploy `6a6dc9ea588b0c0008036312` from source `39ab8649df411262c8109a3c81a57bc38f1e168b`, which is also the rollback target. A later merge, wrong branch/context/ref/tree/artifact, mismatched marker, missing signed-out public proof, or any uncertainty stops. If publication succeeds, the manifest must be re-paused immediately and its production attempt plus the retained live deploy read back separately. The release changes no Firebase, outside-provider configuration, sign-in state, or production data. Build hooks and a reusable protected Netlify publication path remain unverified. No source test clears the current Pages custom-domain claim, configures #133, or deploys #136; those remain separate provider states. The App Engine synchronization script is another surface that must be documented as active or retired.
...
flowchart TD
    ...
    Merge -. "#623 only: exact-parent one-shot under review" .-> WebGate{"Temporary authority and exact artifact valid?"}
    WebGate -- "No" --> Stop
    WebGate -- "Yes" --> WebOnly["Publish pinned web-only artifact"]
    WebOnly --> Netlify
    Netlify --> WebReadback{"Exact marker and signed-out checks pass?"}
    WebReadback -- "No" --> WebRollback["Restore recorded prior deploy"]
    WebReadback -- "Yes" --> WebRepause["Disable temporary authority and verify no replacement"]
```

Text alternative: ordinary merges run CI and do not publish Netlify. Production remains the completed #473 artifact unless the one exact #623 parent, frozen source, artifact, and marker all pass. #623 then requires signed-out public readback; failure restores the recorded prior deploy, while success immediately disables the temporary authority and verifies that the production artifact was not replaced. The separate protected workflow still requires approval and verified Firebase before publishing its Pages copy.
...
| `memberDirectoryEntries/{uid}` | MEMBERS-DIRECTORY-001B/#506 server-only minimum search projection. Its internal document key joins current source records but is never returned; the record contains only a versioned current display name, domain-separated name-prefix digests, one opaque entry reference, optional thumbnail version, and timestamps. Missing, opted-out, or malformed current source state deletes or withholds the entry. |
...
### 8.0q Bounded officer name search and voluntary-thumbnail gallery — SOURCE ONLY, NOT LIVE

MEMBERS-DIRECTORY-001B [#506](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/506) adds the second source slice of parent [#504](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/504): a separate `/admin/member-directory` page where a currently verified website admin explicitly submits a name prefix and can visually compare only the display names and optional thumbnails that people chose to share. It is not the official membership roster, cannot change a role or record, and performs no image-as-query, face detection, facial recognition, embedding, similarity scoring, or biometric processing.

```mermaid
flowchart LR
    Person["Signed-in person"] --> Choice{"Current preference on?"}
    Member["Current bounded display name"] --> Reconcile["Server reconciles current truth"]
    Photo["Optional current processed thumbnail"] --> Reconcile
    Choice -- "No, missing, or malformed" --> Delete["Delete or withhold minimum projection"]
    Choice -- "Yes" --> Reconcile
    Reconcile --> Projection["Server-only name-prefix digest projection\ninternal UID key never returned"]
    Officer["Verified admin enters a name prefix"] --> Search["App Check, authorization, normalization, rate limit, and no-store search"]
    Search --> Projection
    Search --> Current["Re-read current member, preference, and photo state"]
    Current --> Cards["At most 24 current display-name cards\nwith optional processed thumbnails"]
    Search --> Audit["One minimal audit event\nno query or result identities"]
    Browser["Any browser, including admin"] -. "direct access denied" .-> Projection
    Cards -. "not proof" .-> Authority["Membership, role, payment, registration, or official roster"]
```

Text alternative: a server-only projection makes a current opted-in display name searchable by its normalized prefixes; a verified admin submits one bounded name query, and the server re-reads current source records before returning at most 24 voluntary name/thumbnail cards. Browsers cannot directly read the projection, and a card proves no membership or authority.

Projection existence is derived state, not consent or authority. Enabling the #505 preference transactionally reconciles the current member name and optional thumbnail into the projection; disabling it removes the projection before reporting success. Photo replace/remove keeps the same visibility choice while reconciling only the current thumbnail version. An idempotent member-write trigger is a reconciler: it transactionally re-reads the current member, preference, photo, and projection instead of trusting possibly delayed event payloads, so an old name event cannot resurrect an opted-out entry or overwrite newer state. Its retry policy repeats transient background failures safely; malformed or invalid current source state resolves by deleting/withholding the entry instead of poisoning retries. A missing or malformed preference, an invalid current name, or malformed current photo state fails hidden. The internal projection document key may be the Auth UID because it is required to join current server records, but that key is denied by Rules and never enters the callable response, browser logs, analytics, URLs, or audit payloads.

Name matching uses normalization version 1: NFKC, locale-independent lowercase, and Unicode letter/mark/number tokenization, with controls, format characters, and unpaired surrogates rejected. A display name and its canonical form are each capped at 200 UTF-16 code units, making the proven 272-digest projection bound total for every accepted name. The server stores only deduplicated, domain-separated SHA-256 prefix digests. Those stable digests and the opaque entry reference are linkable pseudonyms, not anonymization. The raw submitted query is held only for the request; it is not persisted in the projection, rate state, audit, logs, URL, or analytics. Callable `searchMemberDirectory` requires native App Check and the exact current verified-admin compatibility role before parsing a strict `{ requestId, query }` object, adds private no-store response headers, normalizes and validates the query on the server, requires 2–80 normalized UTF-16 code units, and rejects punctuation-only input. It allows at most 30 searches per verified-admin account in one hour, queries at most 48 candidates, exposes no cursor or total, and returns at most 24 cards.

The projection supplies candidates only. In the same bounded transaction that creates the search audit, the server re-reads each candidate's current member, preference, and photo records, derives every returned name and thumbnail from those current records, and checks that the current normalized name still matches. Stale or malformed candidates are withheld. All reads happen before the audit create. A search transaction ordered after a completed opt-out conflicts/retries as needed and observes the hidden state. A search that committed before opt-out remains a pre-opt-out snapshot and its network response cannot be retroactively recalled; no-store prevents reusable cache storage but not an officer seeing or capturing an already-authorized result. #110 notice/removal wording must disclose that boundary. The audit document ID is a domain-separated digest of actor UID and UUID request ID; it is checked before the candidate query, so one reused command receives a fixed duplicate denial and creates no second audit rather than replaying retained personal results. The event contains only actor UID, fixed action and purpose, request ID, query-length bucket, result count, fixed outcome, and trusted time. Search rate state uses a separate domain-separated account pseudonym and the existing TTL-dependent private limiter. Trigger `syncMemberDirectoryEntryOnMemberWrite` and the #505 profile mutations both reconcile this same current-state contract.

The preserved connected branch sends nothing while the person types. Search happens only after the explicit action and never uses the existing `/admin/members` browser-side full-account filter. The client accepts only the exact bounded response projection, binds pending work to the current application/admin identity, clears stale results when input or identity changes, disables browser autocomplete, and supplies loading, empty, fixed-failure, missing-photo, and narrow-screen states without implying membership. Firestore Rules deny every direct browser read or write to directory entries, preferences, photos, nested records, search audits, and rate state.
...
WEB-002C [#623](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/623) freezes only that inert interface into source `c2d87d1f69f15e128a0bc9b1b9f915b7c8417aec`, tree `411aa6ec9a9459f5d923030533ffc7c007fe6908`, and a 62-file artifact with digest `d837272a1e5efc1575809e87f532276b38d1a63f1dd79ec1aef0533f6da8afb1`. Its one-shot Netlify release is under review, not published. Production still serves #473 deploy `6a6dc9ea588b0c0008036312` from source `39ab8649df411262c8109a3c81a57bc38f1e168b`. Protected-layout proof remains synthetic. Any later public proof must stay signed out and is limited to the exact revision, normal sign-in and administrator guards, and absence of a member-directory request. #623 has no Firebase, provider, account, sign-in, or production-data authority, and #507 remains **NOT AVAILABLE YET**.
