...
| Hosting and release | Netlify currently answers `runmprc.com`. GitHub Pages still reports the same custom domain, so its default URL redirects to the Netlify-served name instead of providing an independent copy. #135 source stops ordinary automatic releases, pauses Git-triggered Netlify production builds, and removes the Pages CNAME from future protected artifacts. After an overbroad WEB-002A artifact was rolled back, [#473](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/473) prepares one replacement web-only exception: its manifest names exact `main` parent `dee7951`, bounded source commit `39ab864` and tree `d76b496`, 60 files, artifact digest `07b10c7d…`, restored production artifact source `ed1b0833`, and rollback deploy `6a6dc219a8136300081811db`. Every mismatch fails closed. | `netlify.toml`, `config/netlify-production-release.json`, `scripts/netlify-release-policy.js`, `.github/workflows/deploy.yml`, `public/404.html` | **PENDING REVIEW AND RELEASE.** Production serves exact rollback source `ed1b0833` until the replacement #473 control merge publishes and its public marker/page checks pass. The replacement adds only static Shop content and Events/Calendar failure containment. It restores no event data, deploys no Firebase or provider change, and does not complete the general protected host under #133/#136/WEB-001. |
...
The former workflow automatically published Pages before attempting Firebase and could finish green after skipping Firebase. #135 replaces that source path with a manual exact-current-commit request, exact latest CI checks, one fixed backend plan, protected short-lived identity wiring, provider readback, and Firebase-before-Pages publication. Missing authority or failed/partial verification is red. Ordinary Git-triggered Netlify production builds stop. Replacement #473 continues only for one production `main` merge whose first parent is exact `dee79511b6e371329aa129139729e112e7a51aad`, then independently fetches and verifies source commit `39ab8649df411262c8109a3c81a57bc38f1e168b` and tree `d76b496cdc5e79015bc048cb961758abbe88b9ce` before building and matching the fixed 60-file digest. Review must also compare executable code against the live artifact source `ed1b0833`; provenance alone does not authorize accumulated source. A later merge, disabled manifest, wrong branch/context/ref/tree/artifact, build-hook metadata, or any uncertainty stops. The pinned build receives no React/provider or server credential environment, and its release PR must build the same exact artifact as production. The authorization is exact-merge scoped rather than durable one-time state. Build hooks and a reusable protected Netlify publication path remain unverified. No source test clears the current Pages custom-domain claim, configures #133, or deploys #136; those remain separate provider states. The App Engine synchronization script is another surface that must be documented as active or retired.
