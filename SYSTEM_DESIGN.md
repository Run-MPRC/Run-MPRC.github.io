...
The former workflow automatically published Pages before attempting Firebase and could finish green after skipping Firebase. #135 replaces that source path with a manual exact-current-commit request, exact latest CI checks, one fixed backend plan, protected short-lived identity wiring, provider readback, and Firebase-before-Pages publication. Missing authority or failed/partial verification is red. Ordinary Git-triggered Netlify production builds stop. Replacement #473 continued only for one production `main` merge whose first parent was exact `dee79511b6e371329aa129139729e112e7a51aad`, then independently fetched and verified source commit `39ab8649df411262c8109a3c81a57bc38f1e168b` and tree `d76b496cdc5e79015bc048cb961758abbe88b9ce` before matching the fixed 60-file digest. Review also compared executable code against live artifact source `ed1b0833`; provenance alone did not authorize accumulated source. The now-inactive manifest, a later merge, wrong branch/context/ref/tree/artifact, build-hook metadata, or any uncertainty stops. The pinned build received no React/provider or server credential environment, and the preview and production artifacts matched. Build hooks and a reusable protected Netlify publication path remain unverified. No source test clears the current Pages custom-domain claim, configures #133, or deploys #136; those remain separate provider states. The App Engine synchronization script is another surface that must be documented as active or retired.
...
The page sends nothing while the person types. Search happens only after the explicit action and never uses the existing `/admin/members` browser-side full-account filter. The client accepts only the exact bounded response projection, binds pending work to the current application/admin identity, clears stale results when input or identity changes, disables browser autocomplete, and supplies loading, empty, fixed-failure, missing-photo, and narrow-screen states without implying membership. Firestore Rules deny every direct browser read or write to directory entries, preferences, photos, nested records, search audits, and rate state.

This source has no production index, Rules, Function, website, privacy-notice, retention/backup approval, or live proof. [#507](https://github.com/Run-MPRC/Run-MPRC.github.io/issues/507) must complete the #110 policy decision, #133 protected short-lived authority, any required index and backend-first deployment/readback (including the trigger retry policy), isolated synthetic staging checks, website publication, `runmprc.com` verification, and backup-officer procedure before use. It must also own an idempotent, dry-runnable projection repair/backfill for any later schema/normalization upgrade, or prove no older preference data exists for the initial target. A green source build or deploy preview is not authorization to search a real name or inspect a real photo.
