...
flowchart TD
    ...
    Merge -. "Only active exact-parent manifest" .-> WebGate{"Pinned source commit and tree match?"}
    WebGate -- "No" --> Stop
    WebGate -- "Yes" --> WebOnly["Publish pinned web-only artifact"]
    WebOnly --> Netlify
```

Text alternative: ordinary merges run CI and do not publish Netlify; the temporary web-only gate accepts only the exact manifest merge and pinned source tree, while the separate protected workflow still requires approval and verified Firebase before publishing its Pages copy.
