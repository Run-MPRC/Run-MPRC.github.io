...
| Hosting and release | Netlify currently answers `runmprc.com`. GitHub Pages still reports the same custom domain, so its default URL redirects to the Netlify-served name instead of providing an independent copy. #135 source stops ordinary automatic releases, pauses Git-triggered Netlify production builds, and removes the Pages CNAME from future protected artifacts. On 2026-08-01, #473's first temporary exception published over-broad deploy `6a6dc016`, the prepared Git rollback restored exact prior source `ed1b0833` as deploy `6a6dc219`, and merge `dee79511` disabled the manifest without replacing that rollback deploy. | `netlify.toml`, `config/netlify-production-release.json`, `scripts/netlify-release-policy.js`, `.github/workflows/deploy.yml`, `public/404.html` | Production is the verified rollback artifact. Its Shop and Events pages still expose raw permission failures. #473 remains open for a new exact Shop/public-error source that is **NOT AVAILABLE YET**. None of these controls deploy Firebase, change a provider setting or production data, or complete the general protected host under #133/#136/WEB-001. |
...
The former workflow automatically published Pages before attempting Firebase and could finish green after skipping Firebase. #135 replaces that source path with a manual exact-current-commit request, exact latest CI checks, one fixed backend plan, protected short-lived identity wiring, provider readback, and Firebase-before-Pages publication. Missing authority or failed/partial verification is red. Ordinary Git-triggered Netlify production builds stop. #473 temporarily armed that gate for a pinned web artifact, but independent review found its source broader than the accepted Shop/Events scope after it had published. The prepared exact-source Git projection restored the previous artifact, and a two-file follow-up disabled the manifest; the public marker and provider readback still identify that rollback. Any future #473 attempt must start from the exact live source and add only the separately reviewed Shop/public-error delta, with a new source commit/tree/digest and new preview, rollback, repause, and live proof. Until then it is **NOT AVAILABLE YET**. Build hooks and a reusable protected Netlify publication path remain unverified. No source test clears the current Pages custom-domain claim, configures #133, or deploys #136; those remain separate provider states. The App Engine synchronization script is another surface that must be documented as active or retired.
...
flowchart TD
    ...
    Merge -. "Ordinary Git production build paused" .-> Netlify["Netlify / runmprc.com\nverified rollback artifact live"]
    Merge -. "Inactive #473 manifest" .-> WebStop["Do not publish"]
    Future["Future narrow #473 source\nNOT AVAILABLE YET"] -. "new preview, rollback, and approval required" .-> WebGate{"Exact parent, source, tree, and artifact match?"}
    WebGate -- "No" --> Stop
    WebGate -- "Yes" --> Netlify
```

Text alternative: ordinary merges and the inactive #473 manifest do not publish Netlify; the verified rollback artifact remains live, and any narrower #473 release needs a new exact source, preview, rollback, approval, and live proof, while the separate protected workflow still requires approval and verified Firebase before publishing its Pages copy.
