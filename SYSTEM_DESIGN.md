...
flowchart TD
    ...
    T["Matching closed transition commitment"] --> Gate
    ...
    S -. "No Stripe request" .-> F["Future C4B pre-send child"]
```

Text alternative: one exact saved safe candidate, matching transition commitment, and fresh active lease create only a durable attempt-2 authorization; C4A may bind its immutable plan, but a separate C4B pre-send boundary is still required.
